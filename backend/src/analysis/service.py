...
class AnalysisService:
    ...
    async def analyze_playlist(
        self, playlist: SPlaylistCreate, version_id: UUID, task_id: UUID
    ) -> float:
        ...
        playlist_tracks = await self.__playlist_tracks(
            playlist.spotify_playlist_id, version_id
        )
        playlist_artists = list(
            chain.from_iterable(track.artists for track in playlist_tracks)
        )

        uniqueness = self.__calculate_uniqueness(
            playlist_tracks, playlist_artists, analysis_settings.weights
        )

        await analyzes.update(
            analysis.id,
            SAnalysisUpdate(status=AnalysisStatus.SUCCESS, uniqueness=uniqueness),
        )
    ...
    async def __set_access_token(self) -> None:
        ...
        if response.status != 200:
            raise Exception(
                f"Failed to get access token. Response status code: {response.status})"
            )
    ...
    async def __make_auth_headers(self) -> dict:
        ...
        if not self._access_token or (
            datetime.now() - self._access_token_creation > timedelta(hours=1)
        ):
    ...
    async def __update_artist(artist_id: str, artist: dict) -> bool | SArtistBase:
        if await artists.is_expired(artist_id):
            artist_db = await artists.update(
                SArtistBase(
                    artist_id=artist_id,
                    name=artist["name"],
        ...
        for id_group in grouped_artists:
            ...
            for artist in response["artists"]:
                ...
                if artist_db:
                    ...
                    artist_db = await artists.create(
                        SArtistBase(
                            artist_id=artist_id,
                            name=artist["name"],
                            genres=artist["genres"],
    ...
    async def __parse_audio_analysis(
        self, tracks_ids_groups: list[str]
    ) -> dict[str, STrackFeaturesBase]:
        audio_analysis = {}
    ...
    async def __parse_tracks(
        self, tracks_json: list[dict], version_id: UUID
    ) -> list[STrack]:
        """
                ...
                if await tracks.is_expired(track_db.track_id):
                    ...
                    await tracks.update(
                        STrackBase(
                            ...
                            track_id=track[
                                "id"
                            ],  # Added because of STrackBase requires it
        ...
        cleaned_tracks = [
            track for track in cleaned_tracks if track["track"]["id"] in tracks_ids
        ]
        ...
        for track_data in cleaned_tracks:
            ...
            for artist_json in track["artists"]:
                await tracks.link_artist(
                    artists_dict[artist_json["id"]], track_db.track_id
                )
    ...
    @staticmethod
    async def __create_playlist_version(
        playlist_info: dict, playlist: SPlaylist, image_url: str
    ) -> tuple[SPlaylistInfo, UUID]:
        playlist_version = await playlist_versions.create(
            SPlaylistVersionBase(
        ...
        return SPlaylistInfo.from_base_and_version(
            playlist, playlist_version, image_url=image_url
    ...
    async def playlist_info(self, playlist_id: str) -> tuple[SPlaylistInfo, UUID]:
        ...
        if playlist:
            if playlist.current_snapshot_id != snapshot_id:
                (
                    playlist_version,
                    playlist_version_id,
                ) = await self.__create_playlist_version(
                    playlist_info, playlist, image_url
                )
                await playlists.update(
                    SPlaylistBase(
                        current_snapshot_id=snapshot_id, spotify_playlist_id=playlist_id
                    )
                )
        ...
        playlist = await playlists.create(
            SPlaylist(spotify_playlist_id=playlist_id, current_snapshot_id=snapshot_id)
        )

        playlist_version, playlist_version_id = await self.__create_playlist_version(
            playlist_info, playlist, image_url
        )
    ...
    def __data_set_uniqueness(values: list[int | float]) -> float:
        ...
        shannon_index = -sum(p * math.log2(p) for p in probabilities if p > 0)

        # Normalize Shannon index (from 0 to 1)
        max_shannon_index = np.log2(len(unique))
        normalized_shannon_index = (
            shannon_index / max_shannon_index if max_shannon_index > 0 else 0
        )
        ...
        uniqueness_score = (
            normalized_shannon_index + normalized_simpson_index + normalized_cv
        ) / 3

        return uniqueness_score

    def __calculate_uniqueness(
        self,
        playlist_tracks: list[STrack],
        playlist_artists: list[SArtist],
        ...
        A = (unique_artists / len(playlist_tracks)) * (
            1 - (np.max(artist_counts) / len(playlist_tracks))
        )
        ...
        danceabilities = [
            track.track_features.dance_ability for track in playlist_tracks
        ]
        ...
        release_dates = [
            track.release_date for track in playlist_tracks
        ]  # track.release_date in date format
        current_date = datetime.now().date()
        years = [t_date.year for t_date in release_dates]
        min_year = min(years)
        T = 1 - (max(years) - min_year) / (
            current_date.year - min_year + 1
        ...
        U = (w1 * P + w2 * A + w3 * M + w4 * G + w5 * T + w6 * E) / (
            w1 + w2 + w3 + w4 + w5 + w6
        )
